...
export const ListItemCart: React.FC<Props> = ({data, addItem, removeItem}) => {
  return (
    ...
    >
      ...
      <View style={{width:'47%'}}>
        <Text style={styles.title}>{`${data?.title.substr(0, 25)}...`}</Text>
        <View style={{flexDirection:'row',alignItems:'flex-end'}}>
          <Text style={{color:colors.light.main,fontWeight:'bold'}}> {data.quantity}x </Text>
          <Text style={{color:colors.light.main,fontWeight:'bold',fontSize:RFValue(14)}}> {data.price.toFixed(2)} </Text>
        </View>
